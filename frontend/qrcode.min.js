 ...
 * Based on qrcodejs by davidshimjs — MIT License
 * https://github.com/davidshimjs/qrcodejs
 *
 * Stripped to just the canvas renderer for this wallet.
 * Usage:
 *   QRCode.draw(canvasElement, "solana:ADDRESS", { size: 200 });
 */
(function(global){
"use strict";
// ── Reed-Solomon + QR core (minimal subset) ─────────────────────────────────
...
var QRMath={};
QRMath.EXP_TABLE=new Array(256);QRMath.LOG_TABLE=new Array(256);for(var i=0;i<8;i++){QRMath.EXP_TABLE[i]=1<<i;}for(var i=8;i<256;i++){QRMath.EXP_TABLE[i]=QRMath.EXP_TABLE[i-4]^QRMath.EXP_TABLE[i-5]^QRMath.EXP_TABLE[i-6]^QRMath.EXP_TABLE[i-8];}for(var i=0;i<255;i++){QRMath.LOG_TABLE[QRMath.EXP_TABLE[i]]=i;}QRMath.glog=function(n){if(n<1){throw new Error("glog("+n+")");}return QRMath.LOG_TABLE[n];};QRMath.gexp=function(n){while(n<0){n+=255;}while(n>=256){n-=255;}return QRMath.EXP_TABLE[n];};
...
var QR8bitByte=function(data){this.mode=QRMode.MODE_8BIT_BYTE;this.data=data;this.parsedData=[];for(var i=0,l=this.data.length;i<l;i++){var byteArray=[];var code=this.data.charCodeAt(i);if(code>0xFFFF){byteArray[0]=0xF0|(code>>>18);byteArray[1]=0x80|((code>>>12)&0x3F);byteArray[2]=0x80|((code>>>6)&0x3F);byteArray[3]=0x80|(code&0x3F);}else if(code>0x7FF){byteArray[0]=0xE0|(code>>>12);byteArray[1]=0x80|((code>>>6)&0x3F);byteArray[2]=0x80|(code&0x3F);}else if(code>0x7F){byteArray[0]=0xC0|(code>>>6);byteArray[1]=0x80|(code&0x3F);}else{byteArray[0]=code;}this.parsedData.push(byteArray);}this.parsedData=Array.prototype.concat.apply([],this.parsedData);if(this.parsedData.length!=this.data.length){this.parsedData.unshift(191);this.parsedData.unshift(187);this.parsedData.unshift(239);}};
 ...
 * @param {object} [opts] - { size: 200, dark: '#000', light: '#fff', margin: 2 }
 */
function drawQR(canvas, text, opts) {
  ...
  // Find the smallest type number that fits.
  var ecl   = QRErrorCorrectLevel.M;
  var typeN = 1;
  for (var t = 1; t <= 40; t++) {
    var rsBlocks = QRRSBlock.getRSBlocks(t, ecl);
    var cap = 0;
    rsBlocks.forEach(function(b){ cap += b.dataCount; });
    // 8-bit mode: 4 bits mode + length bits + 8 per char
    var needed = 4 + QRUtil.getLengthInBits(QRMode.MODE_8BIT_BYTE, t) + text.length * 8;
    if (needed <= cap * 8) { typeN = t; break; }
  }

  var qr = new QRCodeModel(typeN, ecl);
  qr.addData(text);
  qr.make();

  var mc   = qr.getModuleCount();
  var cell = Math.floor(size / (mc + margin * 2));
  if (cell < 1) cell = 1;
  var total = cell * (mc + margin * 2);
  canvas.width  = total;
  canvas.height = total;
