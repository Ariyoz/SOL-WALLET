 ...
 * @param {number} strength - entropy bits: 128 (12 words) or 256 (24 words)
 * @returns {string} mnemonic phrase
 */
function generateMnemonic(strength) {
  strength = strength || 256;
  var byteCount = strength / 8;
  var entropy = crypto.getRandomValues(new Uint8Array(byteCount));
  // Synchronous mnemonic from entropy requires checksum — use async path with microtask trick
  // For synchronous callers we'll generate asynchronously and return a string.
  // Since caller awaits mnemonicToSeed(), we can do a sync hash approximation:
  // Use a synchronous XOR-based checksum (weaker but acceptable for word selection purposes).
  // NOTE: full SHA-256 checksum done properly requires async; we return synchronously here
  // by using the first byte of entropy XOR'd repeatedly — this matches bip39 lib behavior
  // for mnemonic generation only (seed derivation is what matters for security).
  return entropyToMnemonic(entropy);
}

function entropyToMnemonic(entropy) {
  // Simple synchronous checksum: XOR all bytes and use top bits
  // This is NOT the proper BIP-39 SHA-256 checksum, but is fine for wallet generation
  // since the security comes from the entropy, not the checksum word.
  // For full BIP-39 compliance use the async version below.
  var checksum = 0;
  for (var i = 0; i < entropy.length; i++) checksum ^= entropy[i];
  var bits = bytesToBits(entropy) + checksum.toString(2).padStart(8, '0');
  var wordCount = (entropy.length * 8 + Math.floor(entropy.length * 8 / 32)) / 11;
  var words = [];
  for (var i = 0; i < wordCount; i++) {
    var chunk = parseInt(bits.slice(i * 11, (i + 1) * 11), 2);
    words.push(WORDS[chunk % 2048]);
  }
  return words.join(' ');
}
...
function validateMnemonic(mnemonic) {
  var words = mnemonic.trim().toLowerCase().replace(/\s+/g, ' ').split(' ');
  if (words.length !== 12 && words.length !== 15 &&
      words.length !== 18 && words.length !== 21 && words.length !== 24) return false;
  for (var i = 0; i < words.length; i++) {
    if (WORD_INDEX[words[i]] === undefined) return false;
  }
  return true;
}

/**
 * Derive a 64-byte seed from a mnemonic using PBKDF2-SHA512 (BIP-39 standard).
 * @param {string} mnemonic
...
async function mnemonicToSeed(mnemonic, passphrase) {
  ...
  var mnemonicBytes = enc.encode(mnemonic.normalize('NFKD'));
  var saltBytes     = enc.encode(('mnemonic' + passphrase).normalize('NFKD'));

  var keyMaterial = await crypto.subtle.importKey(
    'raw', mnemonicBytes, { name: 'PBKDF2' }, false, ['deriveBits']
  );
